...
public class GunWeaponItem extends Item {
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private static final HumanoidModel.ArmPose RIFLE_POSE = HumanoidModel.ArmPose.create("RIFLE", false, (model, entity, arm) -> {
                Vec3 lookRot = entity.getLookAngle();

                if (arm == HumanoidArm.RIGHT) {
                    model.rightArm.xRot = (model.rightArm.xRot - 1.3F) - (float)lookRot.y;
                } else {
                    model.leftArm.xRot = (model.leftArm.xRot - 1.3F) - (float)lookRot.y;
                }
            });

            private static final HumanoidModel.ArmPose PISTOL_POSE = HumanoidModel.ArmPose.create("PISTOL", false, (model, entity, arm) -> {
                Vec3 lookRot = entity.getLookAngle();
            ...
            public HumanoidModel.@Nullable ArmPose getArmPose(LivingEntity entityLiving, InteractionHand hand, ItemStack itemStack) {
                if(entityLiving.walkAnimation.isMoving()) {
                    return HumanoidModel.ArmPose.EMPTY;
                }
                if(!itemStack.isEmpty() && itemStack.getItem() instanceof GunWeaponItem) {
                    GunWeaponItem gun = (GunWeaponItem) (itemStack.getItem());

                    if(gun.getDualWieldable()) {
                        return PISTOL_POSE;
                    }
                    else {
                        return RIFLE_POSE;
